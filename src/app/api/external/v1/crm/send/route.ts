...
export async function POST(req: NextRequest) {
  ...
  if (!subAccount || !conversationId) {
    return fail('subAccount and conversationId are required.', 400)
  }
  ...
  try {
    const result = await sendMessage(subAccount, {
      conversationId,
    ...
    console.log(
      `[crm-send] ${auth.user.email} sent ${type} to conversation ${conversationId} via ${subAccount} (${message.length} chars)`,
    )
