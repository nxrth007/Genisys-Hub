...
} from '@/components/agent/callback-form'
...
export default function EditTeamCallbackPage(props: {
  params: Promise<{ id: string }>
}) {
  const { id } = use(props.params)
  const query = useQuery<{ callback: Callback }>({
    queryKey: ['team-callback', id],
    queryFn: async () => {
      const res = await fetch(`/api/team/callbacks/${id}`)
