...
export default function TaskBoardPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['notion-tasks', params.id],
    queryFn: async () => {
      const res = await fetch('/api/notion/databases/' + params.id)
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['notion-tasks', params.id] })
    },
  })

  // Parse database schema
  const db = data?.database || {}
  const results: TaskItem[] = data?.results || []
  ...
  const properties = db.properties ? Object.entries(db.properties) as Array<[string, Record<string, unknown>]> : []

  const statusProp = properties.find(([, v]) => v.type === 'status') || properties.find(([name, v]) => v.type === 'select' && name.toLowerCase().includes('status'))
  ...
  let statusOptions: Array<{ name: string }> = []
  if (statusPropType === 'status') {
    statusOptions = ((statusPropDef.status as Record<string, unknown>)?.options as Array<{ name: string }>) || []
  } else if (statusPropType === 'select') {
    statusOptions = ((statusPropDef.select as Record<string, unknown>)?.options as Array<{ name: string }>) || []
  }
  if (statusOptions.length === 0) statusOptions = [{ name: 'No Status' }]
  ...
  const assigneeProp = properties.find(([, v]) => v.type === 'people')?.[0] || properties.find(([name]) => name.toLowerCase().includes('assign'))?.[0]
  const dateProp = properties.find(([, v]) => v.type === 'date')?.[0]
  const descProp = properties.find(([name]) => name.toLowerCase().includes('description') || name.toLowerCase().includes('notes'))?.[0]
  ...
  if (priorityPropDef?.type === 'select') {
    priorityOptions = ((priorityPropDef.select as Record<string, unknown>)?.options as Array<{ name: string }> || []).map(o => o.name)
  }
  ...
  if (assigneePropDef?.type === 'select') {
    assigneeOptions = ((assigneePropDef.select as Record<string, unknown>)?.options as Array<{ name: string }> || []).map(o => o.name)
  }
  ...
  const notionUsers: Array<{ id: string; name: string; email: string }> = usersData?.users || []
  // If assignee is people type, use Notion users as options
  if (assigneePropType === 'people' && notionUsers.length > 0) {
    assigneeOptions = notionUsers.map(u => u.name)
  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 w-full max-w-sm">
            <NewTaskForm
              ...
              onCreated={() => queryClient.invalidateQueries({ queryKey: ['notion-tasks', params.id] })}
            />
          </div>
        </div>
      )}
